...
int main(void)
{
    ...
    g_bldc_motor.pwm_duty = 750;
    g_bldc_motor.target_speed = 300;
	start_motor();                         /* 开启运行 */
...
//    if((g_bldc_motor.pwm_duty > MIN_PWM_DUTY) && (g_bldc_motor.pwm_duty < MAX_PWM_DUTY / 2))
//        timer_channel_output_pulse_value_config(TIMER0,TIMER_CH_3,g_bldc_motor.pwm_duty);
    ...
    temp3 = (float)g_bldc_motor.pwm_duty;
