...
extern float hall_theta_inc;
extern float hall_speed;
extern uint8_t hall_state;
extern float hall_theta_tab[6];
...
void motor_app_task10ms(void)
{
    ...
    {
        motor1.speed_rpm = pmsm_mc_param.hall_speed * ((float)60 / ANGLE_2PI / MOTOR_NOPOLESPAIRS);
        ...
        case RUN:
            {
                ...
                pmsm_mc_param.openloop = 0;
#endif
                pmsm_mc_param.hall_speed = hall_speed;
...
static void motor_ui_init(void)
{
    ...
    LCD_ShowIntNum(175,0,(uint16_t)pmsm_mc_param.hall_speed,5,BLUE,WHITE,24);
